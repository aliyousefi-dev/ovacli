...
export class PlayerInputHostDirective {
  ...
  @Output() hostEvents = new EventEmitter<keyof PlayerHostEvents>(); // --- Activity Listeners (Player Area) --- // When the mouse enters, moves, or a touch occurs, signal the component to show controls. // Note: Touch events like 'touchstart' and 'touchmove' don't require an explicit 'mouseleave' // counterpart; the component handles hiding after a timeout.
  ...
  onUserActivity() {
    this.hostEvents.emit('showControls');
  ...
  onMouseLeave() {
    this.hostEvents.emit('hideControls');
  ...
  handleKeyboardEvent(event: KeyboardEvent) {
    ...
    if (event.key === ' ') {
      ...
      this.hostEvents.emit('playPauseToggle');
      return;
    ...
    if (event.key === 'ArrowRight') {
      ...
      this.hostEvents.emit('stepForward');
      return;
    } else if (event.key === 'ArrowLeft') {
      event.preventDefault();
      this.hostEvents.emit('stepBackward');
      return;
    } // Arrow Keys for Volume Up/Down

    if (event.key === 'ArrowUp') {
      event.preventDefault();
      this.hostEvents.emit('volumeUp');
    } else if (event.key === 'ArrowDown') {
      event.preventDefault();
      this.hostEvents.emit('volumeDown');
    }
  }
}
